...
This research establishes a comprehensive plan for adding Docker container
dependency reporting to the `actions-dependency-submission` GitHub Action. The
implementation is **feasible**, **spec-compliant**, and **non-breaking**.
...
All four patterns use compatible Docker image reference formats and can be
reported using the same PURL structure.
...
- `cd4e78d` - Add comprehensive Docker dependency research and implementation
  plan
